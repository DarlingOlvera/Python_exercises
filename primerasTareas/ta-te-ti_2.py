...
def poner_ficha(matriz,posicion,turno):
    if turno:
        ...
        ficha = 'O'
# Se sustituyo el uso del match case ya que solo es posible usarlo en las versiones de python 3.10 en adelante
# por lo que ahora se esta manejando mediante if y elif
    if posicion == 1:
        if matriz[0][0] == '1':
            matriz[0][0] = ficha
            return 0
...
while jugada < 9:
    ...
    if turno:
        print('Turno del jugador_1 (X), ingresa una posicion')
    else:
        print('Turno del jugador_2 (O), ingresa una posicion')
